...
function attractionsClickable(type)
{
    ...
    attractionsChosen.forEach(attraction => 
        ...
        const marker = new mapboxgl.Marker({ color: 'green' }) // Set the marker color to green
        .setLngLat(coordinates)
        .setPopup(new mapboxgl.Popup()
        .setHTML(`
            <div style="max-width: 300px; border-radius: 10px; overflow: hidden; font-family: Arial, sans-serif;">
                <div style="background-color: green; color: white; padding: 10px; text-align: center;">
                    <h3 style="margin: 0;">${properties.name}</h3>
                </div>
                <div style="padding: 10px; text-align: center;">
                    <img src="${properties.img || ''}" style="max-width: 100%; max-height: 150px; height: auto; border-radius: 5px;" />
                </div>
                <div style="background-color: green; color: white; padding: 10px; text-align: center;">
                    <p style="margin: 0;">${properties.description || ''}</p>
                </div>
                <div style="padding: 10px; text-align: center;">
                    <button style="background-color: green; color: white; border: none; padding: 10px 20px; cursor: pointer; border-radius: 5px;" 
                            onclick="getDirections([${startPoint}], [${coordinates}])">Get Directions</button>
                </div>
            </div>`))
        .addTo(map);
    ...
    "features": [
        // Adding waterfalls
        {
            ...
            "properties": {
                ...
                "img": "./images/bou-sra-waterfall.png"
